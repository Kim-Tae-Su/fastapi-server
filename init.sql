...
-- 1. `CREATE DATABASE` 명령은 이미 Docker Compose의
--    `MYSQL_DATABASE` 환경 변수에 의해 자동으로 처리되므로,
--    이 파일에서 제거합니다.

-- 2. 각 테이블 생성 시, `blog_db.table_name`과 같이
--    데이터베이스를 명시적으로 지정하여
--    테이블이 올바른 위치에 생성되도록 합니다.
--
--    기존 데이터가 있을 수 있으므로 테이블을 삭제하고 다시 생성합니다.
DROP TABLE IF EXISTS blog_db.blog;
DROP TABLE IF EXISTS blog_db.`user`;

-- MySQL의 기본 문자 인코딩 및 정렬 방식을 UTF-8로 설정합니다.
-- `utf8mb4`는 한글을 포함한 다양한 문자를 지원하는 MySQL의 권장 인코딩 방식입니다.
...
CREATE TABLE blog_db.blog (
    ...
    FOREIGN KEY (author_id) REFERENCES blog_db.`user`(id)
) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;

-- author_id에 대한 인덱스 생성
CREATE INDEX author_id_idx ON blog_db.blog(author_id);

-- blog 데이터 삽입
INSERT INTO blog_db.blog (title, author_id, content, modified_dt) VALUES
    ('FastAPI는 어떤 장점이 있는가?', 1,
